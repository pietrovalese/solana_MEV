...
def plot_compare_sandwich_arbitrage_usd(df_sandwich, df_arbitrage, prices):
    ...
    # --- Aggiungi prezzi (solo epoche comuni) ---
    df_merged["price_usd"] = [prices[e] for e in df_merged.index]

    # --- Calcola totali per epoca ---
    ...
    # --- Plot: Revenue in USD (stacked) + Prezzo SOL ---
    fig, ax1 = plt.subplots(figsize=(16, 8))

    # Barre stacked (Sandwich + Arbitrage)
    ...
    print("\n🔹 REVENUE PER EPOCA (solo comuni, USD):\n")
    print(df_merged[[
    ...
    # --- Totali complessivi sulle epoche comuni ---
    ...
    print("\n🎯 SOMMA TOTALE (solo epoche comuni):")
    ...
    print("—" * 60)
    print(f"→ TOTALE:     {total_usd:,.2f} USD  ({total_sol:,.3f} SOL)")

# ================================
# DATI (uguali a quelli precedenti)
# ================================
